...
export class GenerativeAgent {
    ...
    //consturctor
    constructor(name: string, age: number, traits: string, status: string, memory: GenerativeAgentMemory, llm: BaseLLM, dailySummaries: string[] = [], verbose: boolean = false, summaryRefreshSeconds: number = 3600) {
        this.name = name;
        this.age = age;
        this.traits = traits;
        this.status = status;
    ...
    parseList(text: string): string[] {
        ...
        const result: string[] = lines.map((line: string) => line.replace(/^\s*\d+\.\s*/, "").trim());
        return result;
    }
    ...
    async getEntityFromObservations(this: GenerativeAgent, observation: string): Promise<string> {
        const prompt = PromptTemplate.fromTemplate(
            "What is the {entity} doing in the following observation? {observation}"
            + "\nThe {entity} is"
        )

        const result = await this.chain(prompt).call({ //might need to change to call()
    ...
    async getEntityAction(this: GenerativeAgent, observation: string, entityName: string): Promise<string> {
        const prompt = PromptTemplate.fromTemplate(
            "What is the {entity} doing in the following observation? {observation}"
            + "\nThe {entity} is"
        )

        const result = await this.chain(prompt).call({ entity: entityName, observation });
        const trimmedResult = result.output.trim(); //added output
        return trimmedResult;
    }

    async summarizeRelatedMemories(this: GenerativeAgent, observation: string): Promise<string> {
    ...
    private async _generateReaction(this: GenerativeAgent, observation: string, suffix: string, now?: Date): Promise<string> {
        ...
        const current_time_str = (now ? now : new Date()).toLocaleString("en-US", {
            month: "long",
            day: "numeric",
            year: "numeric",
            hour: "numeric",
    ...
    private _cleanResponse(text: string | undefined): string {
        if (text === undefined) {
          return ""; // or handle the case appropriately based on your requirements
    ...
    async generateReaction(this: GenerativeAgent, observation: string, now?: Date): Promise<[boolean, string]> {
        ...
        const fullResult = await this._generateReaction(observation, callToActionTemplate, now=now);
        const result = fullResult.trim().split("\n")[0];
        // AAA
        this.memory.saveContext(
            {},
            {
                [this.memory.addMemoryKey]: `${this.name} observed ${observation} and reacted by ${result}`,
    ...
    async generateDialogueResponse(observation: string, now?: Date): Promise<[boolean, string]> {
        ...
        if (result.includes("GOODBYE:")) {
            ...
            this.memory.saveContext({}, {
            [this.memory.addMemoryKey]: `${this.name} observed ${observation} and said ${farewell}`,
            [this.memory.NowKey]: now,
            });
        ...
        if (result.includes("SAY:")) {
            ...
            this.memory.saveContext({}, {
            [this.memory.addMemoryKey]: `${this.name} observed ${observation} and said ${responseText}`,
            [this.memory.NowKey]: now,
            });
    ...
    async getSummary(this: GenerativeAgent, forceRefresh: boolean = false, now?: Date): Promise<string> {
        // return a descriptive summary of the agent
        const currentTime = new Date();
        // const sinceRefresh = (currentTime.getTime() - this.lastRefreshed.getTime()) / 1000;
        const sinceRefresh = Math.floor((currentTime.getTime() - this.lastRefreshed.getTime()) / 1000);
    ...
    async computeAgentSummary(this: GenerativeAgent): Promise<string> {
        ...
        const result = await this.chain(prompt).call({ name: this.name, relevantMemories: this.memory.relevantMemoriesKey, queries: [`${this.name}'s core characteristics`] })
        return result.output.trim();
    }

    getFullHeader(this: GenerativeAgent, forceRefresh: boolean = false, now?: Date): string {
        // return a full header of the agent's status, summary, and current time.
        now = now ? now : new Date();
        const summary = this.getSummary(forceRefresh=forceRefresh, now=now);
